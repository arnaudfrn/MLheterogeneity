...
vec.pac= c("foreign", "quantreg", "gbm", "glmnet",
           ...
           "caret", "foreach", "multcomp","cowplot", "plyr", "dplyr", "haven", "ggplot2", "dummies")
...
library(haven)
data <- read_dta("data_ML_nonmiss_0626.dta")
data <- as.data.frame(data)


### Collect all the poly variables in one list
library(dplyr)
poly_vars <- tidyselect::vars_select(names(data), starts_with("poly"))
poly_vars <- unname(poly_vars)

# create a categorical poly column generated from the separate poly_* dummy variables. this will be our fixed effect
data$poly <- factor(apply(data[,names(data) %in% poly_vars], 1, function(x) names(x)[x == 1]), levels=poly_vars, ordered=TRUE)


#convert tmt_ML to four dummies
library(dummies)
data <- dummy.data.frame(data, names = "tmt_ML")
data <- data %>% rename(tmt_100 = tmt_ML0 , 
                    tmt_83 = tmt_ML1,
                    tmt_66 = tmt_ML2,
                    tmt_50 = tmt_ML3) %>% filter( tmt_50 == 1 | tmt_100 == 1)


####################################### Inputs  #######################################

sim     <- 90     # number of repetitions
K       <- 2       # number of folds
p       <- 5      # number of groups 
thres   <- 0.20     # quantile for most/least affected group
...
#names <- rep(c("Paid Taxes", "Bribe", "Bribe_amount"),3)   # vector of labels for outcome variables
#Y     <- rep(c("taxes_paid", "bribe_combined", "bribe_combined_amt"), 3)     # vector of outcome variables
#D     <- c( rep("tmt_83", 3), rep("tmt_66", 3), rep("tmt_50", 3))  # vector of treatment variables
names <- c("Paid Taxes") #, "Bribe", "Bribe_amount")   # vector of labels for outcome variables

Y     <- c("taxes_paid") #, "bribe_combined", "bribe_combined_amt")     # vector of outcome variables
D     <- c("tmt_50") #, "tmt_50", "tmt_50")  # vector of treatment variables
...
### NEW CHANGE
fixed_effect <- c("house", poly_vars)         # if no fixed_effect  use    fixed_effect <- "0" -- #Numeric
partition    <- "0"       # if no partition     use    partition    <- "0"


# # create a vector of control variables
controls <- c("roof", "walls", "ravine", "sex_prop", "age_prop", "main_tribe", "employed", "salaried", "work_gov", "job_gov", "know_eachother", "name_know", "house", poly_vars)

affected       <- c("roof", "walls", "ravine", "sex_prop", "age_prop", "main_tribe", "employed", "salaried", "work_gov", "job_gov", "know_eachother", "name_know")      # characteristics for most/least affected analysis
names_affected <- c("roof", "walls", "ravine", "sex_prop", "age_prop", "main_tribe", "employed", "salaried", "work_gov", "job_gov", "know_eachother", "name_know")        # characteristics for most/least affected analysis
...
if(fixed_effect=="0" & cluster=="0"){
  data <- data[,c(Y, D,controls, baseline)]
}

if(fixed_effect=="0" & cluster!="0"){
  data <- data[,c(Y, D,controls, cluster, baseline)]
}

if(fixed_effect!="0" & cluster=="0"){
  data <- data[,c(Y, D,controls, fixed_effect, baseline)]
}

if(fixed_effect!="0" & cluster!="0"){
  data <- data[,c(Y, D, controls, cluster, fixed_effect, baseline)]
}
...
methods      <- c("pcaNNet", "gbm")
method_names <- c("NNet", "Boosting")
...
tune       <- c(20, 20)                                    # number of elements per parameter in the grid. the grid size is tune^{number of tuning parameters}. 
...
tune_param[[2]]  <- 0
#tune_param[[3]]  <- 0
#tune_param[[3]]  <- data.frame(mtry=7)

output_name <- paste("range","-","best", "-", 2, "-" ,2, "-",sim,sep="")
name        <- "EL1"


# WHERE YOU WRITE THE FUNCTION
####################################### Estimation  #######################################
...
r <- foreach(t = 1:sim, .combine='cbind', .inorder=FALSE, .packages=vec.pac) %dopar% { 
  ...
  for(i in 1:length(Y)){
    ...
    datause   <- data.frame(datause_raw[complete.cases(datause_raw[, c(controls, y, d, affected, baseline)]),])
    dataout   <- data.frame(dataout_raw[complete.cases(dataout_raw[, c(controls, y, d, affected, baseline)]),])
    ...
    for(l in 1:length(methods)){
      ...
      fitControl   <- trainControl(method = methodML[l], number = cv[l], repeats = rep[l], allowParallel = FALSE, verboseIter=FALSE, search="random", selectionFunction=select[l])
      arg          <- c(list(form=form, data = datause[ind_nu,],  method = methods[l], tuneGrid = f, trControl = fitControl, preProcess=proces[l], tuneLength=tune[l]), args[[methods[l]]])
      ...
      form1 <- as.formula(paste(y, "~", "B+S+G1+G2+G3+G4+G5 | ", paste(fixed_effect, collapse = " + "), "| 0 |", cluster, sep=""))
      
      a <- tryCatch({
        a <- felm(form1, data=dataout, weights=dataout$weight)  
      },error=function(e){
        cat("ERROR :",methods[l], t, i, "\n")
        form1  <- as.formula(paste(y, "~", "G1+G2+G3+G4+G5 | ", paste(fixed_effect, collapse = " + "), "| 0 |", cluster, sep=""))
      ...
      form1 <- as.formula(paste(y, "~", "B+S+d_ort+S_ort| ", paste(fixed_effect, collapse = " + "), "| 0 |", cluster, sep=""))
      
      a  <- tryCatch({
        a  <- felm(form1, data=dataout, weights=dataout$weight)   
      },error=function(e){
        cat("ERROR2 :",methods[l], t, i, "\n")
        form1 <- as.formula(paste(y, "~", "d_ort+S_ort| ", paste(fixed_effect, collapse = " + "), "| 0 |", cluster, sep=""))
        reg   <- felm(form1, data=dataout, weights=dataout$weight)  
        return(reg)
      }, warning = function(war) {
...
for(i in 1:length(Y)){
  ...
  ggsave(paste(name,"_plot","-",output_name,"-",Y[i],".png",sep=""), p)
  p_best <- plot_grid(result[[best[1]]], result[[best[2]]], ncol=2)
  ggsave(paste(name,"_plot_best","-",output_name,"-",Y[i],".png",sep=""), p_best, width = 10, height = 5)
}
